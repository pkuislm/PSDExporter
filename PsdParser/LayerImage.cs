    ...
    public class LayerImage
    {
        ...
        public byte[] Read()
        {
            if (colorMode is ColorMode.RGB)
                ...
                return ReadCMYK();
            else if (colorMode is ColorMode.Grayscale)
        ...
        byte[] ReadGrayScale()
        {
            ...
            for (int y = 0; y < Height; y++)
            {
                for (int c = 0; c < ChannelImages.Length; c++)
                {
                    ...
                    var channelBuffer = image.ChannelId switch { ChannelId.R => l, ChannelId.TransparencyMask => a, _ => null };
                ...
                {
                    buffer[y * Width * 2 + x * 2 + 0] = l[x];
                    buffer[y * Width * 2 + x * 2 + 1] = a[x];
                }
            }
